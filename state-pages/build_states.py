...
"california": dict(
  ...
  tile=("California<br>Legal Folders","No switchblades, no dirks","california-legal-knives"),
  chip="CA-Legal",
...
"new-york": dict(
  ...
  tile=("New York<br>Legal Folders","No switchblades","new-york-legal-knives"),
  chip="NY-Legal",
  cats=BASE_CATS+[CAT_CAMP, CAT_FILLET],
...
# ---------------------------------------------------------------------------
# HUB DATA — one row per state for the comparison table on /knives/.
# Keep in step with STATES. Values are short on purpose: this table is the
# featured-snippet target, so it has to scan in one pass.
#   verdict classes: "yes" green | "part" amber | "no" red
# ---------------------------------------------------------------------------
HUB = {
 "michigan":       ("Blade shape decides, not the opener",
                    "None", ("yes","Legal"), ("part","Folders yes, daggers felony"), ("no","No")),
 "florida":        ("No licence needed since July 2023",
                    "None", ("yes","Legal"), ("yes","No licence, 21+"), ("no","No")),
 "california":     ("The strictest of the twelve",
                    "None statewide; LA caps at 3&Prime;", ("no","Illegal 2&Prime;+"), ("part","Folders only"), ("no","No")),
 "new-york":       ("Gravity knives legal since 2019",
                    "None statewide", ("no","Prohibited"), ("part","Must be fully hidden"), ("no","No")),
 "ohio":           ("Automatics legal, cities preempted",
                    "None", ("yes","Legal"), ("yes","Legal"), ("yes","Yes")),
 "texas":          ("5.5&Prime; changes where, not whether",
                    "None; over 5.5&Prime; is location-restricted", ("yes","Legal"), ("yes","Legal"), ("yes","Yes")),
 "pennsylvania":   ("Automatics legal since January 2023",
                    "None", ("yes","Legal"), ("yes","Legal"), ("no","No")),
 "georgia":        ("A blade is a weapon past 12 inches",
                    "Over 12&Prime; needs a licence", ("yes","Legal"), ("yes","Under 12&Prime; free"), ("yes","Yes")),
 "north-carolina": ("Open carry lawful; concealed is the test",
                    "None statutory", ("part","Own &amp; open carry"), ("part","Pocket knives only"), ("part","Not established")),
 "tennessee":      ("Limits gone since 2014",
                    "None", ("yes","Legal"), ("yes","Legal"), ("yes","Yes")),
 "illinois":       ("Automatics need a FOID card",
                    "None statewide; 3&Prime; in public buildings", ("part","FOID holders only"), ("yes","Legal"), ("no","No")),
 "wisconsin":      ("Any knife, open or concealed",
                    "None", ("yes","Legal"), ("yes","Legal, no permit"), ("yes","Yes")),
}
...
def build_hub():
    """The /knives/ hub. Every state page links up to this; it links back down."""
    order = [sl for _, sl in SIBLINGS]

    cards = ""
    for sl in order:
        nm = STATES[sl]["name"]; ab = STATES[sl]["abbr"]
        tag = HUB[sl][0]
        cards += f"""      <a class="mso-st__hubcard" href="/knives/{sl}/">
        <span class="mso-st__hubcard-top">
          <span class="mso-st__hubcard-name">{nm}</span>
          <span class="mso-st__hubcard-ab">{ab}</span>
        </span>
        <span class="mso-st__hubcard-tag">{tag}</span>
        <span class="mso-st__hubcard-go">Shop {nm} &rarr;</span>
      </a>
"""
    ...
    for sl in order:
        nm = STATES[sl]["name"]
        _, blade, auto, conc, pre = HUB[sl]
        rows += f"""          <tr>
            <th scope="row"><a href="/knives/{sl}/">{nm}</a></th>
            <td>{blade}</td>
            <td class="mso-st__{auto[0]}">{auto[1]}</td>
            <td class="mso-st__{conc[0]}">{conc[1]}</td>
            <td class="mso-st__{pre[0]}">{pre[1]}</td>
          </tr>
"""

    cats = "".join(f"""      <a class="mso-st__cat" href="/collections/{cs}/">
        <span class="mso-st__cat-img"><img src="/wp-content/uploads/{img}" alt="{cn}" loading="lazy" width="320" height="240"></span>
        <span class="mso-st__cat-body"><span class="mso-st__cat-name">{cn}</span><span class="mso-st__cat-count">[N] products</span></span>
      </a>
""" for cn, cs, img in BASE_CATS + [CAT_AUTO, CAT_CAMP])
...
""" for bn, bs in BRANDS)
    ...
    ld = {"@context":"https://schema.org","@graph":[
      {"@type":"CollectionPage",
       "@id":"https://www.michigansportsoutdoor.com/knives/#page",
       "name":"Shop Knives by State",
       "description":"Knives that are legal to own and carry in your state, with the carry rules for each. Shipped in 2-3 business days by Michigan Sports Outdoor.",
       "inLanguage":"en-US",
       "isPartOf":{"@type":"WebSite","name":"Michigan Sports Outdoor","url":"https://www.michigansportsoutdoor.com/"},
       "provider":{"@type":"Organization","name":"Michigan Sports Outdoor","url":"https://www.michigansportsoutdoor.com/"},
       "mainEntity":{"@type":"ItemList","numberOfItems":len(order),"itemListElement":[
          {"@type":"ListItem","position":i+1,"name":f"Knives in {STATES[sl]['name']}",
           "url":f"https://www.michigansportsoutdoor.com/knives/{sl}/"}
          for i, sl in enumerate(order)]}},
      {"@type":"BreadcrumbList","itemListElement":[
        {"@type":"ListItem","position":1,"name":"Home","item":"https://www.michigansportsoutdoor.com/"},
        {"@type":"ListItem","position":2,"name":"Shop by State"}]}]}

    return f"""<!-- =========================================================================
     Michigan Sports Outdoor — /knives/ HUB PAGE
     Every state page links up to this; this links back down to all of them.
     That reciprocal structure is what makes the set behave as one asset
     instead of twelve orphans.
...
<div class="mso-st__hero">
  <span class="mso-st__hero-bg"><img src="/wp-content/uploads/banner-knives-by-state.jpg" alt="" loading="eager" width="1600" height="600"></span>
  <div class="mso-st__hero-in">
    <div class="mso-st__inner">
      <span class="mso-st__badge">All 50 states &middot; 2026</span>
      <h1>Shop Knives by State</h1>
      <p>Carry rules change at the state line. Pick yours and we will show you what is legal to own and carry there &mdash; and leave out what is not. Shipped in 2&ndash;3 business days.</p>
      <div class="mso-st__ctas">
        <a class="mso-st__btn" href="#compare">Compare All States</a>
        <a class="mso-st__btn mso-st__btn--onDark" href="/collections/edc-folding-knives/">Shop Folding Knives</a>
      </div>
      <p class="mso-st__hero-meta">
        <span>Sold by <strong>Michigan Sports Outdoor</strong></span>
        <span><strong>{len(order)} states</strong> covered so far</span>
        <span>Updated <strong>1 September 2026</strong></span>
      </p>
    </div>
  </div>
...
{TRUST}
  <div class="mso-st__sec">
    <div class="mso-st__sec-head">
      ...
      <p class="mso-st__sec-sub">Each page carries only what is legal to own and carry there</p>
    </div>
    <div class="mso-st__hub">
{cards}    </div>
  </div>

  <div class="mso-st__sec" id="compare">
    <div class="mso-st__sec-head">
      <h2>Knife Carry Rules by State &mdash; Compared</h2>
      <p class="mso-st__sec-sub">The four questions that decide what you can carry</p>
    </div>
    <div class="mso-st__cmp">
      <table>
        <thead>
          <tr>
            <th scope="col">State</th>
            <th scope="col">Blade length limit</th>
            <th scope="col">Automatics</th>
            <th scope="col">Concealed carry</th>
            <th scope="col">State preemption</th>
          </tr>
        </thead>
        <tbody>
{rows}        </tbody>
    ...
    <p class="mso-st__cmp-note">
      &ldquo;State preemption&rdquo; means cities and counties cannot set knife rules stricter than state law. Where it says No, check your city ordinance as well &mdash; that is where most surprises come from. Each state page carries the statutes and the sources. General information only; Michigan Sports Outdoor is a retailer, not a law firm.
    </p>
  </div>
  ...
  <div class="mso-st__sec">
    <div class="mso-st__sec-head">
      <h2>Shop by Category</h2>
      <p class="mso-st__sec-sub">Hunting, camping and everyday carry, shipped nationwide</p>
    </div>
    <div class="mso-st__grid mso-st__grid--6">
{cats}    </div>
  </div>
...
{brands}    </div>
  ...
  <div class="mso-st__capture">
    <h3>We&rsquo;ll tell you when a rule changes</h3>
    <p>Knife law moves &mdash; four of these twelve states changed theirs in the last decade. One email from Michigan Sports Outdoor when yours does, plus first look at season deals.</p>
    <form class="mso-st__capture-form" action="[YOUR FORM ENDPOINT]" method="post">
      <label for="st-email" style="position:absolute;left:-9999px">Email address</label>
      <input type="email" id="st-email" name="email" placeholder="your@email.com" required>
      <button type="submit" class="mso-st__btn">Sign Up</button>
...
<div class="mso-st-sticky" id="stSticky" hidden>
  <p>Not sure what is legal where you live?</p>
  <a class="mso-st__btn" href="#compare">Compare States</a>
  <button class="mso-st-sticky__x" type="button" aria-label="Dismiss">&times;</button>
</div>
...
(function () {{
  var root = document.querySelector('.mso-st');
  var bar = document.getElementById('stSticky');
  var trigger = root && root.querySelector('.mso-st__hub');
  if (!root || !bar || !trigger || !('IntersectionObserver' in window)) return;
  bar.hidden = false;
  var off = false;
  bar.querySelector('.mso-st-sticky__x').addEventListener('click', function () {{
    off = true; bar.classList.remove('is-on');
...
<!-- CollectionPage with an ItemList of every state page, plus BreadcrumbList.
     Set Yoast / Rank Math schema to None for this page. -->
<script type="application/ld+json">
{json.dumps(ld, indent=2)}
</script>
"""
